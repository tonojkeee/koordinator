    ...
    return (
        ...
        >
            <div className="flex items-center gap-4 p-4 pl-5">
                <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${colorMap[toast.type]} flex items-center justify-center text-white shrink-0 shadow-lg shadow-black/5`}>
                    {iconMap[toast.type]}
                </div>
                <div className="flex-1 min-w-0 py-0.5">
                    <h4 className="font-bold text-sm tracking-tight">{toast.title}</h4>
                    {toast.message && <p className="text-current text-xs font-medium opacity-80 mt-0.5 leading-relaxed">{toast.message}</p>}
                </div>
                {toast.action && (
                    <button
                        onClick={(e) => {
